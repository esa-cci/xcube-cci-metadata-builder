...
from xcube_cci.vdcaccess import VECTOR_DATA_CUBE_TYPE
from xcube_cci.dtaccess import DATATREE_TYPE
from xcube_cci.sessionexecutor import OpendapTimeoutError
...
def check_for_processing(dataset, summary_row, time_range):
    ...
    try:
        try:
            ...
            index = 0
            var = vars[index]
            while 'grid_mapping_name' in dataset[var].attrs:
                index += 1
                var = vars[index]
        except IndexError:
            summary_row['open_bbox(3)'] = 'no'
            comment_1 = f'Failed at getting variable from list ' \
...
def check_write_to_disc(summary_row, comment_2, data_id, time_range, variables,
    ...
    try:
        ...
        # test_ds = store.open_data(
        #     ds_id=data_id,
        #     time_range=time_range,
        #     var_names=variables,
        #     bbox=region
        # )
        local_ds_id = write_data(
            dataset, data_id=local_ds_id, store_id='local_data_store'
        )
...
def check_for_support(data_id):
    ...
    for not_supported_id, reason in not_supported_list:
        if isinstance(not_supported_id, list):
            ...
            # supported = False
            # break
        # elif 'esacci.SEALEVEL.mon.IND.MSL.multi-sensor.multi-platform.MERGED.2-0.r1' not in data_id:
            supported = False
            break
...
def test_open_ds(data_id, store, lds, results_csv, store_name):
    ...
    for data_type_for_data in data_types_for_data:
        if DATASET_TYPE.is_super_type_of(data_type_for_data):
            ...
            break
        elif VECTOR_DATA_CUBE_TYPE.is_super_type_of(data_type_for_data):
            data_type = data_type_for_data
            break
        elif DATATREE_TYPE.is_super_type_of(data_type_for_data):
            data_type = data_type_for_data
            break
    ...
    try:
        print(
            f'[{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] Describing data of '
            f'data_id {data_id}.')
        data_descriptor = store.describe_data(data_id=data_id,
                                              data_type=data_type)
    except (DataStoreError, KeyError, OpendapTimeoutError):
    ...
    try:
        ...
        summary_row['Dataset-Title'] = ""

    open_params = dict(data_id=data_id)

    if "place_names" in data_descriptor.open_params_schema.properties:
        places = [data_descriptor.open_params_schema.properties.get("place_names").items.enum[0]]
        open_params["place_names"] = places
    ...
    open_params["variable_names"] = var_list
    ...
    try:
        ...
        if isinstance(dataset, xr.DataTree):
            ...
            dataset = dataset.get(key).to_dataset()
        vars_in_dataset = []
        for var in var_list:
...
def main():
    ...
    add_local_store(
        root="out/",
        store_id="local_data_store",
        persist=False
    )

    lds = get_store("local_data_store")
    set_output_store("local_data_store")

    start_time = datetime.now()

    data_types = ["dataset", "vectordatacube", "datatree"]
    store = new_data_store(store_name)
    for data_type in data_types:
        print(f"Handling data of type '{data_type}'")
        support_file_name = f'{date_today}_test_{store_name}_{data_type}_data_support'
        results_csv = f'{store_name}/{support_file_name}.csv'
        if test_mode:
            results_csv = f'{results_dir}/{support_file_name}.csv'
        data_ids = store.list_data_ids(data_type=data_type)
