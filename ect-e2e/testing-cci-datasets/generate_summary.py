...
def create_json_of_ids_with_verification_flags(data_sets, data_type, results_dir):
    ...
    with open(f'{results_dir}/'
              f'{DATE_TODAY}_{data_type}_DrsID_verification_flags.json',
              'w') as f:
...
def main():
    ...
    data_types = ["vectordatacube", "datatree", "dataset"]
    for data_type in data_types:
        ...
        create_json_of_ids_with_verification_flags(test_data_sets, data_type, results_dir)
    ...
    print(f'[{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] '
            f'Test run finished on {start_time}.')
    print(f'[{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] '
